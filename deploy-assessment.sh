...
# ═══════════════════════════════════════════════════════════════════════
# deploy-assessment.sh — Deploy Assessment Builder
# ═══════════════════════════════════════════════════════════════════════
#
# Usage: ./deploy-assessment.sh test
#        ./deploy-assessment.sh prod
#
# EB Application: metro-builder
# Environments:   Metro-builder-env (test), metro-builder-prod
# S3 prefix:      builder/
# ═══════════════════════════════════════════════════════════════════════

readonly APP_NAME="metro-builder"
readonly S3_BUCKET="elasticbeanstalk-eu-west-1-643502197318"
readonly S3_PREFIX="builder"
...
case "${ENV_ARG}" in
    ...
    prod)
        ...
        ;;
    journeys-test|journeys-prod|journeys*)
        echo "ERROR: This script deploys to metro-builder, NOT journeys-builder."
        echo "       Use deploy-journeys.sh for journeys-builder environments."
        exit 1
        ;;
    *)
...
VERSION_LABEL="assessment-${ENV_ARG}-${TIMESTAMP}-${GIT_HASH}"
...
cd "${SCRIPT_DIR}"
TAG_NAME="deploy/assessment-${ENV_ARG}/${TIMESTAMP}"
git tag -a "${TAG_NAME}" -m "Deploy assessment ${ENV_ARG}: ${VERSION_LABEL}" 2>/dev/null && \
